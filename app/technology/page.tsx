...
/* ─── Machine catalogue (display only) ─── */

const machineCatalog = [
  { name: "CNC Fresatrice 5-Assi", abbr: "CN", powerKw: 7.5,  tempC: 85  },
  { name: "Pick & Place SMT",       abbr: "PP", powerKw: 2.1,  tempC: 45  },
  { name: "Reflow Oven IR",         abbr: "RF", powerKw: 12.0, tempC: 260 },
  { name: "AOI Saki BF-Comet",      abbr: "AO", powerKw: 0.8,  tempC: 40  },
  { name: "Stampante 3D SLS",       abbr: "3D", powerKw: 3.5,  tempC: 200 },
  { name: "Laser Cutter CO₂",       abbr: "LC", powerKw: 5.0,  tempC: 60  },
  { name: "Saldatrice a Onda",      abbr: "SW", powerKw: 8.0,  tempC: 250 },
  { name: "Flying Probe Tester",    abbr: "FP", powerKw: 0.5,  tempC: 35  },
];
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {i40Pillars.map(({ icon, title, desc, metric }) => (
            <div key={title} className="glow-card rounded-xl p-6">
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold" style={{ background: "rgba(34,197,94,.08)", border: "1px solid rgba(34,197,94,.15)", color: "#4ade80" }}>
                ✓ {metric}
              </div>
            </div>
          ))}
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "rgba(201,164,76,.06)" }}>
                  ...
                  <th className="text-center px-6 py-4 text-xs font-bold uppercase tracking-wider text-red-400/80">Macchine Tradizionali</th>
                  <th className="text-center px-6 py-4 text-xs font-bold uppercase tracking-wider" style={{ color: "#4ade80" }}>MetaFan Ottimizzate</th>
                  <th className="text-center px-6 py-4 text-xs font-bold uppercase tracking-wider" style={{ color: "#c9a44c" }}>Miglioramento</th>
                </tr>
              </thead>
              <tbody>
                {comparisonData.map(({ category, traditional, optimized, saving, insight }, i) => (
                  <tr key={category} style={{ background: i % 2 === 0 ? "rgba(255,255,255,.01)" : "rgba(255,255,255,.03)", borderBottom: "1px solid rgba(201,164,76,.06)" }}>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="px-3 py-1 rounded-lg text-xs font-medium" style={{ background: "rgba(239,68,68,.08)", color: "#f87171" }}>{traditional}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="px-3 py-1 rounded-lg text-xs font-bold" style={{ background: "rgba(34,197,94,.08)", color: "#4ade80" }}>{optimized}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
          ...
          <div className="md:hidden space-y-4">
            {comparisonData.map(({ category, traditional, optimized, saving, insight }) => (
              <div key={category} className="rounded-xl p-5" style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(201,164,76,.12)" }}>
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-14">
          ...
          <p className="text-sm text-[#8899b4] max-w-xl mx-auto mt-3">
            ...
            La gestione operativa è disponibile nell&apos;area riservata.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
          {machineCatalog.map((m) => (
            ...
            >
              {/* Icon */}
              <div
                className="w-14 h-14 rounded-xl flex items-center justify-center text-sm font-black mb-3 flex-shrink-0"
                style={{ background: "rgba(201,164,76,.12)", color: "#c9a44c", border: "1px solid rgba(201,164,76,.2)" }}
              >
                {m.abbr}
              </div>

              <h4 className="font-bold text-white text-xs mb-2 leading-tight">{m.name}</h4>

              {/* Status badge */}
              <span
                className="inline-flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-1 rounded-full mb-3"
              ...
              <div className="w-full grid grid-cols-2 gap-1.5">
                <div className="rounded-lg p-2 text-center" style={{ background: "rgba(201,164,76,.05)" }}>
                  ...
                  <p className="text-xs font-bold text-[#c9a44c]">{m.powerKw} kW</p>
                </div>
                <div className="rounded-lg p-2 text-center" style={{ background: "rgba(201,164,76,.05)" }}>
                  <p className="text-[9px] text-[#8899b4] uppercase">Temp. Max</p>
                  <p className="text-xs font-bold text-[#c9a44c]">{m.tempC} °C</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {certifications.map(({ name, desc }) => (
              <div key={name} className="flex items-center gap-4 rounded-xl p-5" style={{ background: "rgba(201,164,76,.05)", border: "1px solid rgba(201,164,76,.12)" }}>
                <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 text-sm font-black" style={{ background: "rgba(201,164,76,.15)", color: "#c9a44c" }}>✓</div>
                <div>
